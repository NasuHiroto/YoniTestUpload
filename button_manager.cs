...
public class button_manager : MonoBehaviour
{
    ...
    double x, response, total;  //問題の経過時間、反応時間、合計時間を記録する変数
    int n;  //問題番号を記録する
    ...
    public void ButtonClick(string button_name)
    {
        switch (button_name)
        {
            case "Button_A":
                ...
                CheckNextSlide();   //次のスライドを表示
                SetStartTime(); //経過時間の取得
                Judge("A"); //回答を
            ...
            case "Button_D":
                ...
                Judge("D");
                break;
            case "Button_All":  //次の章に遷移する場合など
                //Debug.Log("ボタンAllが押されました。");


                NonJudge(); //正誤判定を行わない時間の記録
                director_c.task_image_indx++;   //次のスライド番号を指定
                SetStartTime();
                //A-Dの選択のとき
                if (task_c.read_Answer[director_c.task_image_indx] != "0")
                {
                    director_c.Flugs_gaze_image = false;    //注視画像をスキップするために表示させないようにする
    ...
    private void Is_Continue()
    {
        if (!director_c.Flugs_continue)//最初からはじめた場合
        {
            ...
            n = 1;
            SetStartTime(); //
            gaze_c.Start_Gazelist_Write();//時間０に
